...
class BotConfigurationService:
    ...
    CHOICES: dict[str, list[ChoiceOption]] = {
        ...
        'YOOKASSA_SAVE_PAYMENT_METHOD': [
            ChoiceOption('on_request', '🔘 По запросу'),
            ChoiceOption('always', '✅ Всегда сохранять'),
            ChoiceOption('never', '🚫 Никогда'),
        ],
        'MULENPAY_LANGUAGE': [
    ...
    SETTING_HINTS: dict[str, dict[str, str]] = {
        ...
        'YOOKASSA_SAVE_PAYMENT_METHOD': {
            'description': (
                'Режим сохранения платёжного метода для рекуррентных списаний. '
                '«Всегда» — карта сохраняется при каждом платеже. '
                '«По запросу» — метод сохраняется только при явном указании. '
                '«Никогда» — карта не сохраняется.'
            ),
            'format': 'Выберите один из вариантов.',
            'example': 'on_request',
            'warning': 'Для рекуррентных платежей необходим хотя бы один сохранённый метод.',
            'dependencies': 'YOOKASSA_RECURRING_ENABLED',
        },
        'YOOKASSA_RECURRING_ENABLED': {
            'description': 'Включает автоматическое рекуррентное списание при продлении подписки.',
            'format': 'Булево значение.',
            'example': 'false',
            'warning': (
                'Рекуррентные платежи требуют сохранённых карт и настроенных autopay_enabled/autopay_days_before '
                'в подписке пользователя.'
            ),
            'dependencies': 'YOOKASSA_SAVE_PAYMENT_METHOD, YOOKASSA_ENABLED',
        },
        'YOOKASSA_RECURRING_CHECK_INTERVAL_MINUTES': {
            'description': 'Интервал проверки подписок для рекуррентных списаний (в минутах).',
            'format': 'Целое число не меньше 10.',
            'example': '60',
        },
